...
This example verifies our numerical implementation and examines the accuracy and numerical behavior of the coupling algorithm. A beam is attached at its root to the mass in a spring-mass-damper system, 
...
The beam is modeled by BeamDyn and the time integrator for the spring-damper-mass system is Adams-Bashforth-Moulton (ABM4). 
...
In the present study, the system has quiescent initial conditions with an initial displacement of $0.1$ m in the positive $Z$ direction (see Figure ~\ref{fig:CoupledSystem}). 
...
Moreover, by introducing numerical damping in the BeamDyn module, the maximum stable time increments also increase for most cases.
...
For verification, we analyzed this case in ANSYS using 20 BEAM188 elements (and a point mass with spring and damper) and the time increment was $10^{-5}$ s. 
...
For the BeamDyn results, the beam was discretized in by a single $5^{th}$-order element, and the time increment was  
...
The convergence of the time integrator is also evaluated using the RMS error defined in Eq.~\eqref{RMSdefi}. Again, a free spinning case which uses FAST as a driver code is studied for the convergence, see Figure~\ref{fig:5MWdt_rms}. The blade is discretized by a single fifth-order element and the benchmark solution is obtained  with a time step size of $0.001$ seconds. Second-order convergence rate can be observed in the plot.
...
\begin{figure}
    \centering
\includegraphics[clip,width=5.0in]{\directory  5MW_Coupled_dt.pdf}
...
\begin{figure}
    \centering
\includegraphics[clip,width=5.0in]{\directory  5MW_Coupled_Mesh.pdf}
...
\begin{figure}
    \centering
    \begin{tabular}{c}
    \subfloat[Flap Displacement]{\label{fig:5MWTipX}\includegraphics[clip,width=5.0 in]{\directory  B1TipTDxr.pdf}} \\
\subfloat[Edge Displacement]{\label{fig:5MWTipY}\includegraphics[clip,width=5.0in]{\directory  B1TipTDyr.pdf}}\\
\subfloat[Axial Displacement]{\label{fig:5MWTipZ}\includegraphics[clip,width=5.0in]{\directory  B1TipTDzr.pdf}}\\
\end{tabular}
...
\begin{figure}[h!t]
    \centering
    \begin{tabular}{c}
    \subfloat[$F_{flap}$]{\label{fig:5MWRootFx}\includegraphics[clip,width=3.0 in]{\directory  B1RootFxr.pdf}} \qquad
\subfloat[$F_{edge}$]{\label{fig:5MWRootFy}\includegraphics[clip,width=3.0in]{\directory  B1RootFyr.pdf}}\\
\subfloat[$F_{axial}$]{\label{fig:5MWRootFz}\includegraphics[clip,width=3.0 in]{\directory  B1RootFzr.pdf}} \qquad
\subfloat[$M_{edge}$]{\label{fig:5MWRootMx}\includegraphics[clip,width=3.0in]{\directory  B1RootMxr.pdf}}\\
\subfloat[$M_{falp}$]{\label{fig:5MWRootMy}\includegraphics[clip,width=3.0 in]{\directory  B1RootMyr.pdf}} \qquad
\subfloat[$Torque$]{\label{fig:5MWRootMz}\includegraphics[clip,width=3.0in]{\directory  B1RootMzr.pdf}}
\end{tabular}
